...
# Класс для отрисовки графики в игре
class Drawing:
    def __init__(self, sc, sc_map, player, clock):
        # Инициализация параметров и ресурсов
        self.sc = sc
        self.sc_map = sc_map
        self.player = player
        self.clock = clock
    ...
    def mini_map(self, player):
        ...
        pygame.draw.line(self.sc_map, YELLOW, (map_x, map_y), (map_x + 12 * math.cos(player.angle), map_y + 12 * math.sin(player.angle)), 2)
        pygame.draw.circle(self.sc_map, RED, (int(map_x), int(map_y)), 5)
        for x, y in mini_map:
            pygame.draw.rect(self.sc_map, DARKBROWN, (x, y, MAP_TILE, MAP_TILE))
        self.sc.blit(self.sc_map, MAP_POS)

    # Функция отрисовки оружия игрока
    ...
    # Функция отображения эффекта выстрела
    def bullet_sfx(self):
    ...
    def menu(self):
        ...
        start = button_font.render('НАЧАЛО', 1, pygame.Color('lightgray'))
        button_start = pygame.Rect(0, 0, 450, 150)
        button_start.center = HALF_WIDTH, HALF_HEIGHT
        exit = button_font.render('ВЫХОД', 1, pygame.Color('lightgray'))
        button_exit = pygame.Rect(0, 0, 450, 150)
        button_exit.center = HALF_WIDTH, HALF_HEIGHT + 200

        by_kiper_label_font = pygame.font.Font('font/font1.otf', 20)
        by_kiper_label = by_kiper_label_font.render('НЕ НАЖМИАТЬ', 1, pygame.Color('white'))
        by_kiper_rect = by_kiper_label.get_rect()
        by_kiper_rect.bottomright = WIDTH - 100, HEIGHT - 20
        ...
        while self.menu_trigger:
            ...
            self.sc.blit(start, (button_start.centerx - 170, button_start.centery - 50))

            pygame.draw.rect(self.sc, BLACK, button_exit, border_radius=25, width=10)
            self.sc.blit(exit, (button_exit.centerx - 150, button_exit.centery - 50))

            color = randrange(40)
            label = label_font.render('DOOMPy', 1, (color, color, color))
            self.sc.blit(label, (15, -30))

            self.sc.blit(by_kiper_label, by_kiper_rect)
            ...
            if button_start.collidepoint(mouse_pos):
                ...
                if mouse_click[0]:
                    pygame.quit()
                    sys.exit()
            elif by_kiper_rect.collidepoint(mouse_pos) and mouse_click[0]:
                for _ in range(100):
