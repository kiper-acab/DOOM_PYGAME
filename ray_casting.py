...
# Функция для отображения координат на сетку тайлов
def mapping(a, b):
    return int(a // TILE) * TILE, int(b // TILE) * TILE


# Основная функция рейкастинга
def ray_casting(player_pos, player_angle, world_map):
    ...
    for _ in range(NUM_RAYS):
        ...
        for _ in range(0, WORLD_WIDTH, TILE):
            depth_v = (x - ox) / cos_a
            yv = oy + depth_v * sin_a
        ...
        for _ in range(0, WORLD_HEIGHT, TILE):
            depth_h = (y - oy) / sin_a
            xh = ox + depth_h * cos_a
...
# Функция для получения информации о стенах и их отрисовки
def ray_casting_walls(player, textures):
    ...
    # Итерация по всем лучам
    for ray, casted_values in enumerate(casted_walls):
        depth, offset, proj_height, texture = casted_values
        
        # Определение размеров текстуры стены
